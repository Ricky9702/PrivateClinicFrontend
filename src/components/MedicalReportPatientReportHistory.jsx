...
const MedicalReportPatientReportHistory = () => {
    ...
    const [fromDate, setFromDate] = useState(sessionStorage.getItem("fromDate") || "");
    ...
    const loadReportList = async () => {
        try {
            ...
            if (patientId !== "") {
                apiUrl += apiUrl.includes("?") ? `&patientId=${patientId}` : `?patientId=${patientId}`;
                sessionStorage.setItem("patientId", patientId)
            }

            if (fromDate !== "") {
                apiUrl += apiUrl.includes("?") ? `&fromDate=${fromDate}` : `?fromDate=${fromDate}`;
                sessionStorage.setItem("fromDate", fromDate)
            }

            if (toDate !== "") {
                apiUrl += apiUrl.includes("?") ? `&toDate=${toDate}` : `?toDate=${toDate}`;
                sessionStorage.setItem("toDate", toDate)
            }
            console.info(apiUrl)

            let medicalReports = await Apis.get(apiUrl)
            ...
            sessionStorage.setItem("reportList", JSON.stringify(medicalReportsWithDetails));
        } catch (ex) {
            console.error(ex)
        }
    ...
    const currentItems = reportList !== null && reportList.slice(indexOfFirstItem, indexOfLastItem);
    ...
    return (
        <Container>
            ...
            {reportList === null ? (<p>Loading...</p>)
                : reportList.length === 0 ? (<Alert variant="primary" className="mt-3">Bệnh nhân không có lịch sử khám bệnh...</Alert>)
                    : (
                        <Table striped hover className="my-5">
                            ...
                            <tbody>
                                {Array.isArray(currentItems) && currentItems.map((r) => (
                                    <tr key={r.id} >
                                        <td>{r.id}</td>
