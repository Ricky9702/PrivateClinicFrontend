...
const MedicalReportPatient = () => {
    ...
    const loadPatientList = async () => {
        try {
            const res = await axios.get(`http://localhost:8080/Clinic/api/scheduledetails/`);
                       ...
                       <tbody>
                         {patientList.map((p) => (
                           <tr key={p.id} >
                             ...
                             <td>
                              <Button variant="primary" className="mx-1" onClick={() => onViewReportHistoryClick(p.id)} >Lịch sử bệnh</Button>
                              <Button variant="primary" className="mx-1" onClick={() => onWriteMedicalReportClick(p.id)}>Kê toa</Button>
                             </td>
                           </tr>
                         ))}
                       </tbody>
